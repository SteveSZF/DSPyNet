...
------------------------------------------
local eps = 1e-6
local trainCache = paths.concat(opt.save, 'trainCache.t7')
local testCache = paths.concat(opt.save, 'testCache.t7')
...
local modelL1, modelL2, modelL3, modelL4, modelL5, modelL6, modelL7, modelL8, modelL9
local modelL1path, modelL2path, modelL3path, modelL4path, modelL5path, modelL6path, modelL7path, modelL8path, modelL9path
local down1, down2, down3, down4, down5, down6, down7, down8
local warpmodel2, warpmodel3, warpmodel4, warpmodel5, warpmodel6, warpmodel7, warpmodel8
...
if opt.level > 5 then
   ...
   down5 = nn.SpatialAveragePooling(2,2,2,2):cuda()
   warpmodel5 = createWarpModel():cuda()
...
if opt.level > 6 then
   ...
   down6 = nn.SpatialAveragePooling(2,2,2,2):cuda()
   warpmodel6 = createWarpModel():cuda()
...
if opt.level > 7 then
   ...
   down7 = nn.SpatialAveragePooling(2,2,2,2):cuda()
   warpmodel7 = createWarpModel():cuda()
...
if opt.level > 8 then
   ...
   down8 = nn.SpatialAveragePooling(2,2,2,2):cuda()
   warpmodel8 = createWarpModel():cuda()
...
local inputSize 
local outputSize 
if opt.level % 2 == 1 then
  inputSize = {8, opt.fineHeight, opt.fineWidth}
  outputSize = {2, opt.fineHeight, opt.fineWidth}
else
  inputSize = {2, opt.fineHeight, opt.fineWidth}
  outputSize = {2, opt.fineHeight*2, opt.fineWidth*2}
end
...
local function makeData(images, flows)
  ...
  elseif opt.level == 2 then
	local coarseImages = image.scale(images, opt.fineWidth, opt.fineHeight)
        initFlow = computeInitFlowL1(coarseImages:resize(1,coarseImages:size(1), 
              coarseImages:size(2), coarseImages:size(3)):cuda()):squeeze():float()	
	flowDiffOutput = scaleFlow(flows, opt.fineHeight*2, opt.fineWidth*2)

  elseif opt.level == 3 then
    local coarseImages = image.scale(images, opt.fineWidth/2, opt.fineHeight/2)
    initFlow = ScaleFlowL2(coarseImages:resize(1,coarseImages:size(1), 
              coarseImages:size(2), coarseImages:size(3)):cuda()):squeeze():float()
    flowDiffOutput = scaleFlow(flows, opt.fineHeight, opt.fineWidth)
  ...
  elseif opt.level == 5 then
    local coarseImages = image.scale(images, opt.fineWidth/2, opt.fineHeight/2)
    initFlow = ScaleFlowL4(coarseImages:resize(1,coarseImages:size(1), 
              coarseImages:size(2), coarseImages:size(3)):cuda()):squeeze():float()
    
    flowDiffOutput = scaleFlow(flows, opt.fineHeight, opt.fineWidth)
    flowDiffOutput = flowDiffOutput:add(flowDiffOutput, -1, initFlow)

  elseif opt.level == 6 then
    local coarseImages = image.scale(images, opt.fineWidth, opt.fineHeight)
    initFlow = computeInitFlowL5(coarseImages:resize(1,coarseImages:size(1), 
              coarseImages:size(2), coarseImages:size(3)):cuda()):squeeze():float()
	flowDiffOutput = scaleFlow(flows, opt.fineHeight*2, opt.fineWidth*2)
    
  elseif opt.level == 7 then
    local coarseImages = image.scale(images, opt.fineWidth/2, opt.fineHeight/2)
    initFlow = ScaleFlowL6(coarseImages:resize(1,coarseImages:size(1), 
              coarseImages:size(2), coarseImages:size(3)):cuda()):squeeze():float()

    flowDiffOutput = scaleFlow(flows, opt.fineHeight, opt.fineWidth)
    flowDiffOutput = flowDiffOutput:add(flowDiffOutput, -1, initFlow)
  elseif opt.level == 8 then
    local coarseImages = image.scale(images, opt.fineWidth, opt.fineHeight)
    initFlow = computeInitFlowL7(coarseImages:resize(1,coarseImages:size(1), 
              coarseImages:size(2), coarseImages:size(3)):cuda()):squeeze():float()
	flowDiffOutput = scaleFlow(flows, opt.fineHeight*2, opt.fineWidth*2)
	
  elseif opt.level == 9 then
    local coarseImages = image.scale(images, opt.fineWidth/2, opt.fineHeight/2)
    initFlow = ScaleFlowL8(coarseImages:resize(1,coarseImages:size(1), 
              coarseImages:size(2), coarseImages:size(3)):cuda()):squeeze():float()
  ...
  if opt.level%2==1 then
	...
	imageFlowInputs = initFlow:float()
  end
