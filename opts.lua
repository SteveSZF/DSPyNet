...
function M.parse(arg)
    ...
    cmd:option('-L5', 'models/modelL5_4.t7', 'Trained Level 5 model')
    cmd:option('-L6', 'models/modelL6_4.t7', 'Trained Level 6 model')
    cmd:option('-L7', 'models/modelL7_4.t7', 'Trained Level 7 model')
    cmd:option('-L8', 'models/modelL8_4.t7', 'Trained Level 8 model')
    cmd:option('-L9', 'models/modelL9_4.t7', 'Trained Level 9 model')

    cmd:option('-netType',     'volcon', 'Lua network file')
    cmd:option('-retrain',     'none', 'provide path to model to retrain with')
    cmd:option('-optimState',  'none', 'provide path to an optimState to reload from')

    cmd:option('-iterSize',  1, 'the real batchsize is batchsize/iterSize,accumulate the gradient,update parameters with gradient/iterSize')
    cmd:text()
